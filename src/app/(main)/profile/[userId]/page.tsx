...
import { useUserProfile, useUserStats, useUserDocuments } from '@/hooks/useUserProfile';
import { Document } from '@/@types/document.type';
import DocumentCard from '@/components/documents/DocumentCard'; // Dùng lại Card
import { useAuthStore } from '@/store/auth.store';
import { useEffect } from 'react';

// Component Header "Chỉ Xem"
function UserProfileHeader({ user, stats }: { user: any; stats: any }) {
    ...
    <>
      <div className="p-8 bg-gradient-to-r from-gray-500 to-gray-700 rounded-lg shadow-lg text-white">
        <div className="flex items-center">
          <img
            src={user.avatarUrl || `https://ui-avatars.com/api/?name=${user.fullName}&background=random&color=fff`}
            alt={user.fullName}
      ...
      <div className="flex gap-6 mt-8">
        <div className="flex-1 p-6 bg-white rounded-lg shadow-md">
          ...
          <div className="mt-2 text-3xl font-bold">{stats?.totalUploads ?? 0}</div>
        </div>
        <div className="flex-1 p-6 bg-white rounded-lg shadow-md">
          <div className="text-sm text-gray-500 uppercase">Total Downloads</div>
          <div className="mt-2 text-3xl font-bold">{stats?.totalDownloads ?? 0}</div>
        </div>
        <div className="flex-1 p-6 bg-white rounded-lg shadow-md">
          <div className="text-sm text-gray-500 uppercase">Avg Downloads/Doc</div>
          <div className="mt-2 text-3xl font-bold">{stats?.avgDownloadsPerDoc ?? 0}</div>
        </div>
      </div>
    </>
  );
      ...
      <div className="mt-12 bg-white rounded-lg shadow-lg">
        ...
        <div>
          ...
          {docData && docData.data.length > 0 ? (
            ...
            !isLoadingDocs && <p className="p-6">Người dùng này chưa đăng tải tài liệu nào.</p>
          )}
        </div>
      </div>
    </div>
