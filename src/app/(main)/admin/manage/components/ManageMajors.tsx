// src/app/admin/manage/components/ManageMajors.tsx
'use client';
...
export default function ManageMajors() {
  ...
  const { data: allSubjects, isLoading: isLoadingSubjects } = useAdminSubjects();
  ...
  useEffect(() => {
    if (editingMajor) {
      setName(editingMajor.name);
      // Map subjects to their IDs
      const subjectIds = editingMajor.subjects.map((s) => s._id);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div>
        <h3 className="text-xl font-semibold mb-4">
          {editingMajor ? `Đang sửa: ${editingMajor.name}` : 'Thêm ngành học mới'}
        </h3>
        <form onSubmit={handleSubmit} className="space-y-4 p-4 border rounded-lg bg-gray-50">
          <div>
            <label className="block text-sm font-medium mb-1">Tên ngành học</label>
            <input
              type="text"
              value={name}
      ...
      <div>
        ...
        <div className="h-96 overflow-y-auto border rounded-md p-4">
          {isLoadingMajors && <p className="text-center text-gray-500">Đang tải...</p>}
